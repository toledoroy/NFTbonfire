...
const personaGetOrMake = async (chain, contract, token_id, force = false) => {
  ...
  */
  ...
  if (!persona) {
    logger.warn("[TEST] personaGetOrMake() Register New Persona Token to DB -- chain:'" + chain + "' token_id:'" + token_id + "' contract:'" + contract + "'  " + JSON.stringify(persona));
    //Full Procedure - Add & Update
    return personaAddToDB(chain, contract, token_id);
  }
  else logger.warn("[TEST] personaGetOrMake() Requested Persona Exists -- chain:'" + chain + "' token_id:'" + token_id + "' contract:'" + contract + "' force Update:'" + force + "'");

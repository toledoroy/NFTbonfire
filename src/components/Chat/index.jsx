...
function Chat(props) {
    ...
    const collection = selectedHash ? NFTCollections[selectedHash] : {};
    ...
    // }, [selectedHash, limit, isAuthenticated]);

    // const loadRooms = (postId) => {
    //     if (isAuthenticated && selectedHash) {    //TODO: Check if User is Allowed to View this Space
    //         //Log
    //         // console.log("[TEST] Chat() RUNNUING W/Hash:"+selectedHash);
    //         //Get Rooms for Space (by Space's hash)
    //         const RoomQuery = new Moralis.Query(Room);
    //         RoomQuery.equalTo("parentId", selectedHash);  //By Hash
    ...
    //             //Log
    //             console.log("Chat() Got " + results.length + " Rooms for Space:" + selectedHash);
    //             if (results && results.length > 0) {
    //                 //Log
    //                 // console.log("[DEBUG] Chat() Got "+results.length+" Rooms for Space:"+selectedHash, results); 
    //                 //Set Rooms
    //                 setRooms(results);
    ...
    //             else {
    //               //Init Rooms
    //               let roomsInit = initRooms(selectedHash); 
    //               //Set Rooms
    //               // setRooms(roomsInit); //Try Without... Also Use Live Query
    //               //Log
    //               // console.log("[TEST] Chat() No Rooms Found for Space:"+selectedHash+" --> Init Rooms", roomsInit);
    //             }//No Rooms
    //             */
    //         });
    //         //Log
    //         // console.log("Moralis Query Object for Current Room: ", {selectedHash, curRoomId});
    //     }//Authenticated
    //     else setRooms([]);
    // }//loadRooms()
            ...
            <div className="chat flex framed">
                <div className="left" style={{ maxWidth: '400px' }}>
                    ...
                    <div key="collections" className={"collections framed count_" + Object.keys(NFTCollections).length}>
                        <h3>{Object.keys(NFTCollections).length} Collections</h3>
                    ...
                    {!!collection &&
                        <div key="rooms" className={"rooms " + collection?.hash + ' : ' + selectedHash}>
                            ...
                            {rooms.map((room, index) => (
                                <Link key={room.id} to={{ pathname: `/chat/${collection.chain}/${collection.hash}/${room.id}` }} key={collection.hash}>
                                    <RoomEntrance key={room.id} hash={collection.hash} collection={collection} room={room} selected={(curRoomId === room.id)} />
                                </Link>
                            ))}
                            {/* </Skeleton> */}
                        </div>
